...
// as DATABASE_URL *before the first deploy*. Better Auth's tables (user, session,
// account, verification) live in THIS database — the app's own Postgres — never
// deploymill's control-plane database. Every supported Postgres provider (the
// house `deploymill` Postgres, Neon, Supabase) injects the SAME env var, so
// nothing here is provider-specific.
//
// Cold-start tolerance: a freshly-provisioned database can take a moment to
// accept connections. So Better Auth's schema migration runs in the BACKGROUND
// with retry (see index.js) and the server starts listening immediately —
// `/healthz` answers 200 right away so the deploy health-gate passes during
// warmup instead of rolling back a perfectly good build.
import pg from "pg";
...
// The Pool is handed directly to Better Auth (`database: pool` in src/auth.js),
// which uses its built-in Kysely Postgres adapter against it. Reuse this same
// pool for any of your own app queries too.
...
/**
 * Retry `fn` a few times with a fixed delay. Used for boot-time schema migration
 * so a database that's still accepting its first connections doesn't fail the
 * deploy.
 */
